...
export type Detrbridge2TodoStatus = "open" | "done";

const TODO_STATUSES: readonly Detrbridge2TodoStatus[] = ["open", "done"];

export function normalizeDetrbridge2Status(value: string): Detrbridge2TodoStatus {
  return (TODO_STATUSES as readonly string[]).includes(value)
    ? (value as Detrbridge2TodoStatus)
    : "open";
}

export interface Detrbridge2TodoComment {
  id: string;
  todoId: string;
  body: string;
  author: string;
...
export interface Detrbridge2TodoAttachment {
  id: string;
  todoId: string;
  fileName: string;
  sizeBytes: number;
...
export interface Detrbridge2TodoItem {
  id: string;
  title: string;
  assignee: string;
  /** ISO date (yyyy-mm-dd) or null when the todo has no deadline. */
  dueDate: string | null;
  status: Detrbridge2TodoStatus;
  comments: Detrbridge2TodoComment[];
  attachments: Detrbridge2TodoAttachment[];
  createdAt: string;
  updatedAt: string;
}

export interface Detrbridge2TodosResult {
  source: SourceState;
  errorMessage?: string;
  items: Detrbridge2TodoItem[];
}

export interface Detrbridge2TodoInput {
  title: string;
  assignee: string;
...
const ATTACHMENT_BUCKET = "detrbridge-files2";
...
function toTodoItem(
  ...
  return {
    ...
    status: normalizeDetrbridge2Status(row.status),
    comments,
    attachments,
...
function toComment(row: SupabaseCommentRow): Detrbridge2TodoComment {
  return {
    id: row.id,
    todoId: row.todo_id,
    body: row.body,
...
): Promise<Map<string, Detrbridge2TodoAttachment[]>> {
  const byTodo = new Map<string, Detrbridge2TodoAttachment[]>();
  const { data, error } = await supabase
    .from("detrbridge_todo2_attachments")
 ...
 */
export async function getAllTodos2Admin(): Promise<Detrbridge2TodosResult> {
  const supabase = createServiceClient();
  if (!supabase) return { source: "env-missing", items: [] };
  try {
    const { data, error } = await supabase
      .from("detrbridge_todos2")
      .select(TODO_COLUMNS)
      .order("created_at", { ascending: true });
    if (error) throw error;
    const rows = (data ?? []) as SupabaseTodoRow[];

    const commentsByTodo = new Map<string, Detrbridge2TodoComment[]>();
    let attachmentsByTodo = new Map<string, Detrbridge2TodoAttachment[]>();
    if (rows.length > 0) {
      const { data: commentRows, error: commentError } = await supabase
        .from("detrbridge_todo2_comments")
...
export async function getTodo2ByIdAdmin(id: string): Promise<Detrbridge2TodoItem | null> {
  const supabase = createServiceClient();
...
export async function addAttachment2(
  todoId: string,
  file: File
  ...
  try {
    const { error } = await supabase.from("detrbridge_todo2_attachments").insert({
      todo_id: todoId,
...
export async function deleteAttachment2(
  attachmentId: string
): Promise<MutationResult> {
  const supabase = createServiceClient();
  if (!supabase) {
...
export async function updateTodo2(
  id: string,
  input: Detrbridge2TodoInput
): Promise<MutationResult> {
  const title = input.title.trim();
...
export async function setTodo2Status(
  id: string,
  status: Detrbridge2TodoStatus
): Promise<MutationResult> {
  const supabase = createServiceClient();
  ...
  try {
    const { error } = await supabase
      .from("detrbridge_todos2")
      .update({ status: normalizeDetrbridge2Status(status) })
      .eq("id", id);
    if (error) throw error;
    return { ok: true };
  } catch (error) {
...
export async function deleteTodo2(id: string): Promise<MutationResult> {
  const supabase = createServiceClient();
  if (!supabase) {
    return { ok: false, errorMessage: "Service credentials missing." };
  }
  try {
    const { data: attachmentRows } = await supabase
      ...
      .select("storage_path")
      .eq("todo_id", id);
    const { error } = await supabase.from("detrbridge_todos2").delete().eq("id", id);
    if (error) throw error;
...
export async function addComment2(
  todoId: string,
  body: string,
  author: string
): Promise<MutationResult> {
  ...
  try {
    const { error } = await supabase.from("detrbridge_todo2_comments").insert({
      todo_id: todoId,
      body: text,
...
export async function deleteComment2(commentId: string): Promise<MutationResult> {
  const supabase = createServiceClient();
  if (!supabase) {
    return { ok: false, errorMessage: "Service credentials missing." };
  }
