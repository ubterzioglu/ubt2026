// Geçici toplu koşu — 13 büyük şehirde (ilk turda job başına sadece 4 aday
// bulunmuştu) arama kapsamını genişletir: max_queries 12→24, max_source_urls
// 40→80, max_extract_urls 25→50. tavily2 fallback artık run-job.ts'te devrede,
// birincil Tavily kota/limit hatası verirse otomatik ikinci anahtara geçer.
// Kullanım sonrası silinir. Kullanım: npx tsx scripts/dev-finder-expand-major-cities.ts
  ...
  for (const city of CITIES) {
    for (const templateKey of TEMPLATE_KEYS) {
      ...
      const { data: job, error: jobError } = await supabase
        ...
        .insert({
          title: `${template.label} — ${city} (genişletilmiş tur)`,
